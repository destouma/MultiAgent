...
| **3 — IDE-native integration** ✅ (code-complete) | Editor context-menu actions (**Add selection**, **Explain**); conversation list + per-conversation model/persona pinning (server pinning N/A - only one active connection exists so far); status-bar widget (health + model); conversation search; Markdown/JSON export; auto-include active file/selection as context. | Driven from the editor, not just the tool window; several conversations — the editor-actions/multi-chat/status-bar half is verified in a real sandbox (incl. a persona-loading bug found and fixed: `PersonaRegistry`'s filesystem lookup can't work under `PluginClassLoader`, fixed via bundling personas as classpath resources); **search, export, and the "include active file" auto-context toggle are new, compile and pass `./gradlew test`, but are not yet manually verified in a real sandbox** |

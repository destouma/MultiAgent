...
| **2 — Workspace-assisted chat** ✅ | Auto-bind to the open `Project` (one conversation per project, keyed by `basePath`). Wire `ToolLoopRunner` + `WorkspaceService` + `GitService` + `RunCommandService`. Approval gate → `DialogWrapper`. Tool-activity rows. `VfsUtil.markDirtyAndRefresh` after writes. Checkpoint diff/revert via `DiffManager`. `run_command` output surfaced. | Approval → edits visible in the editor → diff/revert → command output — done, verified in a real sandbox incl. a real `run_command` subprocess run |
| **3 — IDE-native integration** ✅ | Editor context-menu actions (**Add selection**, **Explain**); conversation list + per-conversation model/persona pinning (server pinning N/A - only one active connection exists so far); status-bar widget (health + model); conversation search; Markdown/JSON export; auto-include active file/selection as context. | Driven from the editor, not just the tool window; several conversations — the editor-actions/multi-chat/status-bar half is verified in a real IDE (incl. a persona-loading bug found and fixed: `PersonaRegistry`'s filesystem lookup can't work under `PluginClassLoader`, fixed via bundling personas as classpath resources). A real-IDE testing round (v0.2.0 → v0.2.4, see [`intellij-plugin/README.md`](intellij-plugin/README.md#real-ide-testing)) found and fixed 6 more bugs: message-row layout stretch, a model-combo display bug (3 attempts to actually land), the chat-tab-strip UI review pass turning out to be unusable (reverted to a combo box), a new-conversation model-resolution gap that could send an empty model to the server, and a health dot with no color logic. **Search and export are both now confirmed live too** - every item in this row is verified. |
| **4 — Orchestrator** ✅ | Wire `OrchestratorService` (plan → specialists → synthesize) with progress in the tool window. Coordinator picker + per-specialist models dialog. Specialists write through the same approval-gated loop (already true in `core`). | Orchestrator conversation plans, shows specialist progress, edits the project under the gate — a real run against `intellij-plugin` itself ("what's the architecture?") produced plan → one grounded `researcher` specialist → coherent synthesis, captured via Export as JSON. **Still unverified: a specialist actually writing a file** (every real run so far has been read-only) |
...
- **`intellij-plugin/` has no CI workflow yet** — `.github/workflows/` only covers
  `desktop-java/` (`desktop-java.yml`) and CodeQL. Add a `test` job (`./gradlew test`,
  needs JDK 21) mirroring `desktop-java.yml`'s, so a regression in the forked core or
  the plugin UI is caught on every push/PR instead of only when someone remembers to
  run it locally.

...
| **3 — IDE-native integration** 🚧 | Editor context-menu actions (**Add selection**, **Explain**) ✅; conversation list + per-conversation model/persona pinning ✅ (server pinning N/A - only one active connection exists so far, see Feature status); status-bar widget (health + model) ✅. **Not done: search + export MD/JSON, auto-include active file/selection as context.** | Driven from the editor, not just the tool window; several conversations — done for the editor-actions/multi-chat half, verified in a real sandbox (incl. a persona-loading bug found and fixed: `PersonaRegistry`'s filesystem lookup can't work under `PluginClassLoader`, fixed via bundling personas as classpath resources) |
